 ...
 * 格式化时间戳为相对时间
 */
export function formatTimestamp(timestamp: number, t: (key: string, options?: Record<string, unknown>) => string, i18n: { language: string }): string {
 ...
 * 描述 Cron 表达式
 */
export function describeCron(expression: string, t: any): string {
  ...
  if (dom === "*" && mon === "*" && dow === "*") {
    ...
    if (mon !== "*") segments.push(monNames[mon] || `${mon}${t("workspace.cronDescMonthSuffix")}`);
 ...
 * 描述间隔时间 (毫秒)
 */
export function describeIntervalMs(ms: number, t: any): string {
 ...
 * 格式化 Cron 时间戳
 */
export function formatCronTimestamp(ms: number | null): string {
  if (ms === null) return "-";
 ...
 * 格式化 Cron 相对时间
 */
export function formatCronRelativeTime(ms: number | null, t: any): string {
  if (ms === null) return "-";
  const now = Date.now();
  const diff = ms - now;
 ...
 * 描述调度
 */
export function describeSchedule(schedule: any, t: any): string {
  if (!schedule) return "-";
